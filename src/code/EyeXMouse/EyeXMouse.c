...
const float FDeadZone = 8.0;  // pixels
const float g_SlowZone = 80; // pixels
const float FSpeed = 0.24f; // how fast to move when far away
const float FSlowSpeed = 0.12f; // now fast to move when close
...
void HandleNewCoords(float x, float y)
{
	...
	if (abs(lastRawX - x) + abs(lastRawY - y) > 20)
	{
		lastRawX = x;
		lastRawY = y;
		return;//reject noise
